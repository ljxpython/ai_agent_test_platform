...
Prereq: start the full stack via docker compose (nginx at `http://localhost`):

```bash
docker compose -f infra/docker-compose.yml up -d --build
```

Install Playwright browser once (local dev machines):
...
Notes:
- Config: `ui/playwright.config.ts` (defaults `E2E_BASE_URL=http://localhost`).
- The test forces `apiUrl=/api/lg` and `assistantId=sql_agent` via URL params.
